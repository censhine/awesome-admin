...
export default {
  ...
  actions: {
    ...
    apply ({ state, commit }, refresh) {
      ...
      if (refresh) {
        commit('awadmin/page/keepAliveClean', null, { root: true })
        router.replace('/refresh')
      }
    },
    /**
     * @description 确认已经加载组件尺寸设置 https://github.com/aw-projects/aw-admin/issues/198
     * @param {Object} context
     */
    isLoaded ({ state }) {
    ...
    load ({ state, dispatch }) {
      return new Promise(async resolve => {
        // store 赋值
        state.value = await dispatch('awadmin/db/get', {
          dbName: 'sys',
          path: 'size.value',
