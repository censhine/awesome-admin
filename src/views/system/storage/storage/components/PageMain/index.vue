<template>
  <div class="aw-p">
    ...
    <el-breadcrumb class="breadcrumb aw-mb" separator-class="el-icon-arrow-right">
      <el-breadcrumb-item>
        <a class="aw-cp" @click="switchFolder(0)">资源管理</a>
      </el-breadcrumb-item>

      <el-breadcrumb-item
        v-for="item in naviData"
        :key="item.storage_id">
        <a class="aw-cp" @click="switchFolder(item.storage_id)">{{item.name}}</a>
      </el-breadcrumb-item>
    </el-breadcrumb>
        ...
        <li v-for="(item, index) in currentTableData" :key="index">
          <dl>
            <dt>
              ...
              <span class="storage-name aw-ml-10" :title="item.name">{{item.name}}</span>
              <el-dropdown
                placement="bottom"
                :show-timeout="50"
                size="small"
