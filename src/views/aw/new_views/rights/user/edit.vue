      ...
      <el-form
        ...
        <el-form-item
          ...
          <el-checkbox-group v-model="checkedRoleIds" @change="handleCheckedRoleChange">
            <el-checkbox v-if="role"  v-for="role in form.roleIds" :label="role.id" :key="role.id">{{role.name}}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
    ...
    methods:{
      ...
      create() {
        this.$refs.addForm.validate((valid) => {
          if (valid) {
            ...
            updateInfo(MODULE_NAME, insertData).then((res)=>{
              if(res.status){
      ...
      isShowDia(parentToChildData, isVisible){
        ...
        getEntityInfo(MODULE_NAME,null,{id:this.id}).then(res=>{
          ...
          this.checkedRoleIds = res.data.roleIds
          console.log(this.checkedRoleIds)
        }).catch(e=>{
          console.log(e)
        })
