...
<script>
  ...
  export default {
    components: {
      ...
      'resetpasswordDialog':()=>import('../../resetPassword'),
      //'rightsDialog':()=>import('../../rights')
    },
    props: {
      loading: {
        default: false
    ...
    methods: {
      ...
      handleResetPassword(index) {
        ...
        this.formData = { ...this.currentTableData[index] }
        ...
        this.$confirm('确定要重置密码吗?', '提示', {
          ...
          .then(() => {
            resetPassword(MODULE_NAME, null, {id: this.formData.id}).then(res => {
              if (res.status > 0) {
                this.$message.success('密码已重置成功，请前往邮箱查收密码')
              }
            }).catch(e => {
      ...
      handleRights(index) {
        ...
        this.$nextTick(() => {
          ...
          this.$refs.rightsDialog.isShowDia(this.formData,true)
        })
      },
      // 弹出编辑对话框
      handleUpdate(index) {
      ...
      handleDelete(val) {
        ...
        this.$confirm('确定要执行该操作吗?', '提示', {
          ...
          .then(() => {
            deleteInfo(MODULE_NAME,{},{id:this.currentTableData[val].id})
              .then(() => {
