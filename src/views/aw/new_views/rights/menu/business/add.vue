    ...
    <el-dialog
      ...
      <el-form
        ...
        <el-form-item
          label="上级菜单"
          prop="parentId">
          <menuCategory ref="menuCategory" api-path="/operate/web/businessMenu/findAll" v-model="form.parentId" @getMenuCategory="getMenuCategory"></menuCategory>
        </el-form-item>
...
<script>
    const MODULE_NAME = 'businessMenu'
    const REFRESH = 'businessMenuListRefresh'
    const SUBMIT = 'businessMenuSubmit'
    ...
    export default {
      ...
      mounted() {
        getListByParentId(MODULE_NAME,{},{parentId:0}).then(res=>{
        ...
        getMenuCategory(val){
          ...
          create(){
            this.$refs.addForm.validate((valid) => {
              if (valid) {
                insertInfo(MODULE_NAME,{...this.form}).then(res=>{
                  if(res.status){
                    ...
                    this.$message.error('提交失败'+res.tips)
                  }
                }).catch(e=>{
