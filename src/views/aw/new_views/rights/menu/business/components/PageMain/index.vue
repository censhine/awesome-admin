      ...
      <el-table-column
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
           <el-tag :type="scope.row.type == '1' ? `success` : `default`">{{menuMap[scope.row.type]}}</el-tag>
          </template>
        </el-table-column>
      <el-table-column
        prop="sort"
  ...
  methods: {
    ...
    handleView(index) {
      ...
      this.$nextTick(() => {
        ...
        this.dialogLoading = false
        this.formData = this.form
        this.$refs.viewDialog.isShowDia({id:index},true)
      })
    },
    // 弹出审核对话框
    handleEdit(index) {
      ...
      this.$nextTick(() => {
        ...
        this.dialogLoading = false
        this.formData = this.form
        this.$refs.editDialog.isShowDia({id:index},true)
      })
    },
    // 请求编辑
    update() {
    ...
    handleDelete(index) {
      ...
      this.$confirm('确定要执行该操作吗?', '提示', {
        ...
        .then(() => {
          deleteInfo(MODULE_NAME,null,{id:index})
            .then((res) => {
